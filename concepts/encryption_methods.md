# 暗号化方式

## 概要
データを安全に伝送するための暗号化技術。共通鍵・公開鍵・ハイブリッドの3種類が実用される。

## 理解したこと

### 共通鍵暗号方式
- 暗号化と復号に同じ鍵を使う
- 高速で処理コストが低い
- **鍵配送問題**：相手に鍵を渡す経路が安全でなければ意味がない。この問題を解決するために公開鍵暗号方式が生まれた
- 代表例：AES、DES

---

### 公開鍵暗号方式

**数学的な原則（RSAの場合）：**
数学的に対になる2つの鍵（A・B）のペアを作る。
- 性質1：Aで閉じればBで開く
- 性質2：Bで閉じればAで開く

この「どちらの鍵で閉じても、もう片方で開ける」という対称的な可逆性は、RSAの数学的性質（指数演算の可換性）に由来する**RSA固有の特徴**であり、公開鍵暗号方式全般に共通する原則ではない。

ECDSAなど他の方式には、この「閉じる/開ける」という暗号化・復号の操作自体が存在しない（[digital_signature.md](digital_signature.md)の「署名の抽象契約」を参照）。ECDSAは署名専用のアルゴリズムであり、暗号化には使えない。

**名前の定義：**
- 自分だけが持つ方 → 「秘密鍵」
- みんなに配る方 → 「公開鍵」

秘密鍵を渡す必要がないため鍵配送問題を解決できる。ただし処理コストが高く遅い。

代表例：RSA

用途による鍵の使い分けは後述の[目的による鍵の使い方の違い](#目的による鍵の使い方の違い)を参照。

```mermaid
sequenceDiagram
    box 受信者
        participant R as 受信者
    end
    box 送信者
        participant S as 送信者
    end

    Note over R: 公開鍵・秘密鍵のペアを生成<br/>秘密鍵は自分だけが保持（外に出さない）
    R->>S: 公開鍵を配布（漏洩しても問題ない）
    Note over S: 公開鍵でメッセージを暗号化
    S->>R: 暗号化されたメッセージ
    Note over R: 秘密鍵で復号<br/>（秘密鍵は外に出ないため漏洩リスクが低い）
```

---

### ハイブリッド暗号方式（実用）
公開鍵暗号の「遅さ」と共通鍵の「鍵配送問題」を両方解決するための組み合わせ。

「共通鍵でやりたいけど、送るときの漏洩が怖い」→ だから送るときだけ公開鍵暗号方式を使う、という発想。

```mermaid
sequenceDiagram
    box 送信者
        participant S as 送信者
    end
    box 受信者
        participant R as 受信者
    end

    Note over S,R: ①公開鍵暗号で共通鍵を安全に交換（遅いが安全）
    S->>R: 公開鍵で暗号化した共通鍵
    Note over S,R: ②以降は共通鍵で暗号化通信（速い）
    S->>R: 共通鍵で暗号化したデータ
```

HTTPSなどの実際の通信はほぼこの方式で動いている。

---

### 目的による鍵の使い方の違い
同じ鍵ペアでも目的によって使い方が逆になる：

| 目的 | 閉じる鍵 | 開ける鍵 |
|---|---|---|
| 暗号化（内容を隠す） | 公開鍵 | 秘密鍵（受信者だけが開ける） |
| デジタル署名（本人証明） | 秘密鍵 | 公開鍵（誰でも検証できる） |

## 関連概念
- [digital_signature.md](digital_signature.md)（同じ鍵ペアを「証明」に使う場合。暗号化との対比）
- [ssl_tls.md](ssl_tls.md)（ハイブリッド暗号方式を実際に使うプロトコル）
- [pki.md](pki.md)（公開鍵が本物かを保証する信頼の仕組み）

## 関連実装
- [passkey_webauthn](../coding/passkey_webauthn/) — ECDSAが暗号化ではなく署名専用アルゴリズムであることを実装コードで確認した

## ソース
- 書籍：イラスト図解式ネットワークの基礎　第6章（2026-05-15）
- https://shikaku-dou.com/fe-lesson-20/（2026-05-15）
- https://www.kagoya.jp/howto/it-glossary/security/ssl/（2026-05-15）
- 2026-09-01・Qiita「デジタル署名アルゴリズム(DSA/ECDSA)」 https://qiita.com/lemiyachi/items/628576b16e8f490de17c

## タグ
暗号化, 共通鍵, 公開鍵, RSA, ハイブリッド暗号, AES, 鍵配送問題, セキュリティ, ECDSA
