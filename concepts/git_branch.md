...
checkout は「HEADが指す先を変える」操作。渡すものによって detached になるかが決まる。

| 引数 | HEADが指すもの | 状態 |
|---|---|---|
| ブランチ名 | ブランチ名（経由してコミットIDへ） | 通常 |
| コミットID | コミットID直接（ブランチを飛ばす） | detached HEAD |

遡るためにコミットIDを直接指定すると、保持する情報がブランチではなくコミットIDになってしまうため detached になる。
...
```mermaid
flowchart TD
    A["git status\nコンフリクトしているファイルを確認"] --> B["ファイルを開いてマーカーを見て\nあるべき姿に編集・マーカーを削除"]
    B --> C["git add ファイル名\n（複数ある場合は1ファイルずつ）"]
    C --> D{"全ファイル片付いた？"}
    D -->|No| B
    D -->|Yes| E["git commit"]
...
- 元のブランチに戻るには `git checkout <元のブランチ名>` でいい（既存ブランチ名を渡せば通常状態に戻る）
