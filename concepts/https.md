...
HTTPは平文通信のため盗聴・改ざんのリスクがある。

TLSを挟むことでHTTPの仕組みを変えずに安全性を追加できる点が革新的。
...
## 関連概念
- ssl_tls（TLSの具体的なハンドシェイク・証明書の仕組み）
- http（HTTPSが安全化する対象のプロトコル）
- encapsulation（TLSがHTTPを包む構造はカプセル化の典型例）
- transport_layer（TCPがTLSの下位に位置し、信頼性を担保する）
- tcp_ip_model（HTTPS通信がどの層で何をするかを示すモデル）
