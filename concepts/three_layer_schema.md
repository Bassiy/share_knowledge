...
同じ「行・列」でも、スキーマ層によって呼び方が変わる。

| 用語セット | 行 | 列 | 対応スキーマ層 |
|---|---|---|---|
| タプル / アトリビュート | タプル | アトリビュート | 概念スキーマ（論理設計） |
| ロー / カラム | ロー | カラム | 内部スキーマ（物理設計） |
| レコード / フィールド | レコード | フィールド | 外部スキーマ（実装フェーズ） |

組み合わせを混ぜると文脈がズレる（タプル/カラム等はNG）。「理論寄り → 実装寄り」の順に並んでいる。

---

## 関連概念
- dbms（DBMSが3層のマッピングを管理する）
- db_design（設計3ステップの各ステップがそれぞれのスキーマ層に対応する）
- normalization（正規化は概念スキーマ層の設計原理）
