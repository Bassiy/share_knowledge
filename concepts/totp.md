...
HMACは暗号化(鍵で元のデータに戻せる)ではなく一方向関数なので、「暗号化して送る」というモデルではない。鍵の受け渡しは登録時の1回だけで、以降は両者が独立に同じ計算をして答え合わせするだけ。
...
sequenceDiagram
    ...
    box ユーザー
        participant U as 認証アプリ
    end

    S->>S: 秘密鍵をランダム生成
    S->>U: 秘密鍵を一度だけ渡す(QRコード/手入力)
    Note over S,U: これ以降、鍵の受け渡しは発生しない

    S->>S: HMAC(秘密鍵, 現在時刻)を計算
    U->>U: HMAC(秘密鍵, 現在時刻)を計算
    Note over S,U: 両者が独立に計算し、結果だけ突き合わせる
```

HMACの一方向性(暗号化との違い)は [hash.md](hash.md) を参照。
